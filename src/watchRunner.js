// İzleme döngüsü — likidite düşüşü (sarı) / rug (kırmızı) — TON / BSC / Solana.
...
function createWatchRunner(deps) {
  ...
  async function checkWatchedTokens() {
    ...
    for (const w of active) {
      const cid = w.chain || 'solana';
      if (!byChain.has(cid)) byChain.set(cid, []);
      byChain.get(cid).push(w);
    }

    for (const [chainId, items] of byChain) {
      const chain = chainsRegistry.getChain(chainId);
      const liveData = new Map();

      for (let i = 0; i < items.length; i += WATCH_BATCH_SIZE) {
        const batch = items.slice(i, i + WATCH_BATCH_SIZE);
        ...
        if (i + WATCH_BATCH_SIZE < items.length) {
      ...
      for (const w of items) {
        ...
        for (const msg of cms) {
          ...
          if (!channelAccepts(ch, chainId)) continue;
          const msgLang = msg.lang || DEFAULT_LANG;

          if (freshToken && freshAudit) {
            const card = formatTokenCardForChain(chainId, freshToken, freshAudit, msgLang, cardLevel, { slim: true });
            const banner = bannerSourceForChain(chainId, cardLevel === 'red' ? 'red' : 'yellow');
            ...
            const bannerText = formatRiskBannerForChain(chainId, {
            ...
            await bot.sendMessage(msg.chatId, wrapEmojis(bannerText, chainId), {
              parse_mode: 'HTML',
        ...
        if (level === 'yellow') {
          ...
          console.log(`   ⚠️ [${chainId}] RISK: $${w.tokenSymbol} liq $${initial.toFixed(0)} → $${current.toFixed(0)}`);
        } else if (level === 'red') {
          storage.updateWatched(w.poolId, { alerted: true, alertedAt: Date.now(), lastWatchLevel: 'red' });
          storage.incStat('scamsCaught');
          console.log(`   🚨 [${chainId}] SCAM/RUG: $${w.tokenSymbol}`);
        }
      }
    }
  }
